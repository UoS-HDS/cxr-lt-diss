"""
ML Decoder classifier. Adapted from
...
def add_ml_decoder_head(
    ...
    """
    Adds ml decoder head to a model.
    :param model: model to add ml decoder head to
    :param num_classes: number of classes, if -1, will use model.num_classes
    :param num_of_groups: number of groups, if -1, will use num_classes
    :param decoder_embedding: embedding size for the decoder
    :param zsl: if 1, will use zero-shot learning mode

    :return: model with ml decoder head
    """
...
class TransformerDecoderLayerOptimal(nn.TransformerDecoderLayer):
    """
    Transformer decoder layer with optimal parameters for the ML Decoder.
    This layer is used in the MLDecoder
    """
...
class GroupFC(object):
    ...
    def __call__(self, h: Tensor, duplicate_pooling: Tensor, out_extrap: Tensor):
...
class MLDecoder(nn.Module):
    ...
    def __init__(
        ...
        if self.zsl:
            ...
            self.decoder.duplicate_pooling_bias = nn.Parameter(torch.Tensor(1))
            ...
            self.decoder.duplicate_pooling_bias = nn.Parameter(
    ...
    def forward(self, x, mask=None):
        ...
        embedding_spatial_786 = nn.functional.relu(
            embedding_spatial_786,
            inplace=True,
        )
        ...
        h = self.decoder(
            tgt,
            embedding_spatial_786.transpose(0, 1),
