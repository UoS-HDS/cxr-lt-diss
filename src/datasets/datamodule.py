"""
lightning data module for cxr data. Adapted from
https://github.com/dongkyunk/CheXFusion/blob/main/dataset/cxr_datamodule.py
"""
...
from src.datasets.cxr_lt_24 import CxrStudyIdDataset, CxrDataset
from src.datasets.vinbig_cxr import VinBigDataset
from src.datasets.nih_cxr import NihDataset
...
class CxrDataModule(pl.LightningDataModule):
    """ "CXR Data Module for training and validation datasets."""

    def __init__(self, datamodule_cfg: dict, dataloader_init_args: dict):
        super().__init__()
    ...
    def setup(self, stage):
        ...
        if stage in ("fit", "validate"):
            ...
            train_idx, val_idx = next(
                msss.split(self.df, self.df[self.cfg["classes"]].values)
            )
            ...
            if self.cfg["predict_pseudo_label"] == "vinbig":
                ...
                self.pred_dataset = VinBigDataset(self.cfg, pred_df, transforms_val)
                ...
                self.pred_dataset = self.dataset_cls(self.cfg, pred_df, transforms_val)
