...
cmd="STAGE=2 uv run --python 3.12.9 main.py fit --config .tmp/configs/medvit+asl+1024/config-stage-2.yaml --model.pretrained_path $backbone_path"
...
CUDA_VISIBLE_DEVICES=0,1 docker run --rm --gpus 2 --ipc=host \
    ...
    bash -c "set -e; cp -r configs/task1/* .tmp/configs/medvit+asl+1024/ && $cmd" 2>&1 | tee logs/docker_${TIMESTAMP}.log
