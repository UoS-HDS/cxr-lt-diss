...
cmd="STAGE=2 uv run --python 3.12.9 main.py predict --config .tmp/configs/medvit+asl+1024/config-stage-2-pred.yaml --ckpt_path $ckpt_path --model.pretrained_path $backbone_path"
...
CUDA_VISIBLE_DEVICES=0 docker run --rm --gpus 1 --ipc=host \
    ...
    $cmd > submissions/task1/medvit+asl+1024/results.txt"
