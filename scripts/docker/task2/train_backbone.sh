...
cmd="STAGE=1 uv run --python 3.12.9 main.py fit --config .tmp/configs/convnext+asl+384/config.yaml --data.datamodule_cfg.use_pseudo_label $use_pseudo"
if [ -n "$ckpt_path" ]; then
...
CUDA_VISIBLE_DEVICES=0,1 docker run --rm --gpus 2 --ipc=host \
    ...
    bash -c "set -e; cp -r configs/task1/* .tmp/configs/convnext+asl+384/ && eval \"$cmd\"" 2>&1 | tee logs/docker_${TIMESTAMP}.log
