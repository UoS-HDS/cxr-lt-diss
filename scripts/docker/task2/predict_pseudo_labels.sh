...
CUDA_VISIBLE_DEVICES=0 docker run --rm --gpus 1 --ipc=host \
    ...
    STAGE=1 uv run --python 3.12.9 main.py predict --config .tmp/configs/convnext+asl+384/config.yaml \
      --data.datamodule_cfg.predict_pseudo_label chexpert \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run --python 3.12.9 main.py predict --config .tmp/configs/convnext+asl+384/config-nih.yaml \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run --python 3.12.9 main.py predict --config .tmp/configs/convnext+asl+384/config-vinbig.yaml \
