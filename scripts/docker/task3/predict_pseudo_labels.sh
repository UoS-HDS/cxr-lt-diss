#!/bin/bash
#SBATCH --job-name=backbone_teacher_predict_task3
...
docker run --rm --gpus 1 --ipc=host \
    ...
    STAGE=1 uv run --python 3.12.9 main.py predict --config /opt/configs/task3/config.yaml \
      --data.datamodule_cfg.predict_pseudo_label chexpert \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run --python 3.12.9 main.py predict --config /opt/configs/task3/config-nih.yaml \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run --python 3.12.9 main.py predict --config /opt/configs/task3/config-vinbig.yaml \
