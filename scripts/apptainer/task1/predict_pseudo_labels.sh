...
apptainer exec --nv \
    ...
    bash -c "uv sync -q && \
    STAGE=1 uv run main.py predict --config configs/task1/config.yaml \
      --data.datamodule_cfg.predict_pseudo_label chexpert \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run main.py predict --config configs/task1/config-nih.yaml \
      --trainer.devices=1 --ckpt_path '$ckpt_path' && \
    STAGE=1 uv run main.py predict --config configs/task1/config-vinbig.yaml \
      --trainer.devices=1 --ckpt_path '$ckpt_path'"
